    ...
    defs += [
        ...
        SettingDef(
            ...
            description=(
                ...
                "THE BAND TO WATCH IS ROUGHLY 10,000 TO 25,000 EMBEDDINGS at 1536 "
                "dimensions: a knowledge base there keeps the project-wide index and "
                "is measurably slower for it. Measured against the previous query "
                "shape on a 150,000-embedding table: a 10,000-row knowledge base went "
                "6.2 ms to 50.4 ms, and a 20,000-row one 4.4 ms to 132.5 ms — exact "
                "either way, where the old answers were recall 0.61 and 0.58. The same "
                "10,000-row knowledge base WITH an index of its own answers in 1.8 ms "
                "at recall 1.000, so at this default it is paying 27x for nothing. If a "
                "project has a knowledge base in that band and its searches feel slow, "
                "lowering this setting for that project is the fix, following the "
                "procedure below. "
